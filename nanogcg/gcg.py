...
class GCGVLM(GCG):
    def __init__(
        ...
        self.embedding_layer = model.get_input_embeddings()
        self.not_allowed_ids = None if config.allow_non_ascii else get_nonascii_toks(processor, device=model.device)
        self.prefix_cache = None
    ...
    def run(
        ...
        # Append the GCG string at the end of the prompt if location not specified
        if not any([("{optim_str}" in t.get("text", "") for t in d["content"]) for d in messages]):
            messages[-1]["content"].append({"type": "text", "text":"{optim_str}"})

        prompt = processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        inputs = processor(images=images, text=prompt, return_tensors="pt").to(model.device, torch.float16)

        template = processor.decode(inputs["input_ids"][0])
        ...
        # Tokenize everything that doesn't get optimized
        before_ids = processor(text=before_str, padding=False, return_tensors="pt")["input_ids"].to(model.device, torch.int64)
        after_ids = processor(text=after_str, add_special_tokens=False, return_tensors="pt")["input_ids"].to(model.device, torch.int64)
        target_ids = processor(text=target, add_special_tokens=False, return_tensors="pt")["input_ids"].to(model.device, torch.int64)
        ...
        if config.use_prefix_cache:
            with torch.no_grad():
                output = model(input_ids=before_ids, pixel_values=inputs['pixel_values'], use_cache=True)
                self.prefix_cache = output.past_key_values
        ...
        for _ in tqdm(range(config.num_steps)):
            ...
            with torch.no_grad():
                ...
                if config.filter_ids:
                    sampled_ids = filter_ids(sampled_ids, processor)
            ...
            optim_ids = buffer.get_best_ids()
            optim_str = processor.batch_decode(optim_ids)[0]
            optim_strings.append(optim_str)

            buffer.log_buffer(processor)                
    ...
    def init_buffer(self) -> AttackBuffer:
        ...
        if isinstance(config.optim_str_init, str):
            init_optim_ids = processor(text=config.optim_str_init, add_special_tokens=False, return_tensors="pt")["input_ids"].to(model.device)
            if config.buffer_size > 1:
                init_buffer_ids = processor(text=INIT_CHARS, add_special_tokens=False, return_tensors="pt")["input_ids"].squeeze().to(model.device)
                init_indices = torch.randint(0, init_buffer_ids.shape[0], (config.buffer_size - 1, init_optim_ids.shape[1]))
                init_buffer_ids = torch.cat([init_optim_ids, init_buffer_ids[init_indices]], dim=0)
            else:
                init_buffer_ids = init_optim_ids
            ...
            if (len(config.optim_str_init) != config.buffer_size):
                logger.warning(f"Using {len(config.optim_str_init)} initializations but buffer size is set to {config.buffer_size}")
            try:
                init_buffer_ids = processor(text=config.optim_str_init, add_special_tokens=False, return_tensors="pt")["input_ids"].to(model.device)
            except ValueError:
                logger.error("Unable to create buffer. Ensure that all initializations tokenize to the same length.")
    ...
    def compute_token_gradient(
        ...
        if self.prefix_cache:
            ...
            input_embeds = torch.cat([self.before_embeds, optim_embeds, self.after_embeds, self.target_embeds], dim=1)
            output = model(inputs_embeds=input_embeds, pixel_values=self.pixel_values)
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                if self.prefix_cache:
                    ...
                    outputs = self.model(inputs_embeds=input_embeds_batch, pixel_values=self.pixel_values.repeat(current_batch_size, 1, 1, 1))
...
# A wrapper around the GCGVLM `run` method that provides a simple API
...
def runVLM(
    ...
    gcg = GCGVLM(model, processor, config)
    result = gcg.run(messages, images, target, bos_token)
    return result